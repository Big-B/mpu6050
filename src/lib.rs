    ...
    D: DelayMs<u8>, 
{
    ...
    /// Get current sample rate. From [MPU6050 Register
    /// Map](https://www.invensense.com/wp-content/uploads/2015/02/MPU-6000-Register-Map1.pdf),
    /// chapter "Sample Rate divider"
    ///
    ...
    /// 7), and 1kHz when the DLPF is enabled (see Register (dec) 26).
    ///
    ...
    pub fn get_rate(&mut self) -> Result<u8, Error<E>> {
        let mut gyro_rate: u8 = 1;
        let config = self.read_u8(CONFIG)?;
        let dlpf: u8 = config & 0x7; // keep last three bits CONFIG[2..0]
        if dlpf == 0 || dlpf == 7 {
            gyro_rate = 8;
        }

        let sample_rate_div = self.read_u8(SMPLRT_DIV)?;
        Ok(sample_rate_div)
    }
